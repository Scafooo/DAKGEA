#!/usr/bin/env bash
# ============================================================
#  Run Massive Synthetic vs Real Data Comparison
#  Executes baseline and augmented experiments in parallel
# ============================================================

set -euo pipefail

# ============================================================
#  CONFIGURATION
# ============================================================
DEFAULT_MODEL="bert_int"  # bert_int or rrea
DEFAULT_JOBS=2
DEFAULT_TIMEOUT=7200
DEFAULT_GPU_ID=0
RUN_BASELINE=true
RUN_AUGMENTED=true
DRY_RUN=false
PATTERN="*.yaml"
RESUME=false

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# ============================================================
#  USAGE
# ============================================================
usage() {
    cat <<EOF
Usage: $0 [OPTIONS]

Run massive synthetic vs real data comparison experiments.

This script runs two modes of experiments:
  Mode 1 (Baseline):  Only original/real aligned pairs (no augmentation)
  Mode 3 (Augmented): Original + synthetic pairs (standard augmentation)

The comparison evaluates:
  - Quality of synthetic data (baseline performance - synthetic gap)
  - Benefit of augmentation (augmented - baseline)

OPTIONS:
    --model MODEL           Model type: bert_int or rrea (default: ${DEFAULT_MODEL})
    --jobs N                Number of parallel jobs (default: ${DEFAULT_JOBS})
    --timeout SECONDS       Timeout per job (default: ${DEFAULT_TIMEOUT})
    --gpu-id ID             GPU device ID (default: ${DEFAULT_GPU_ID})
    --baseline-only         Run only baseline experiments
    --augmented-only        Run only augmented experiments
    --pattern PATTERN       YAML file pattern (default: "*.yaml")
    --resume                Resume interrupted run
    --dry-run               Show what would be executed
    --help, -h              Show this help

EXAMPLES:
    # Run all experiments (baseline + augmented) for bert_int
    $0 --model bert_int --jobs 4

    # Run only baseline experiments for rrea
    $0 --model rrea --baseline-only

    # Dry run to see what will be executed
    $0 --dry-run

    # Resume interrupted run
    $0 --resume

    # Run specific pattern
    $0 --pattern "D_W_15K_V1_*.yaml"

EXPERIMENT DIRECTORIES:
    Baseline:  config/experiments/massive/{model}_baseline/
    Augmented: config/experiments/massive/{model}_aug_red/

RESULTS:
    Results will be saved in results/ directory with experiment-specific names.

    To analyze results after completion, use:
        python experiments/statistics/compare_baseline_augmented.py --model {model}

EOF
    exit 0
}

# ============================================================
#  PARSE ARGUMENTS
# ============================================================
while [[ $# -gt 0 ]]; do
    case "$1" in
        --model)
            DEFAULT_MODEL="$2"
            shift 2
            ;;
        --jobs)
            DEFAULT_JOBS="$2"
            shift 2
            ;;
        --timeout)
            DEFAULT_TIMEOUT="$2"
            shift 2
            ;;
        --gpu-id)
            DEFAULT_GPU_ID="$2"
            shift 2
            ;;
        --baseline-only)
            RUN_BASELINE=true
            RUN_AUGMENTED=false
            shift
            ;;
        --augmented-only)
            RUN_BASELINE=false
            RUN_AUGMENTED=true
            shift
            ;;
        --pattern)
            PATTERN="$2"
            shift 2
            ;;
        --resume)
            RESUME=true
            shift
            ;;
        --dry-run)
            DRY_RUN=true
            shift
            ;;
        -h|--help)
            usage
            ;;
        *)
            echo -e "${RED}Unknown option: $1${NC}"
            usage
            ;;
    esac
done

# Validate model
if [[ "$DEFAULT_MODEL" != "bert_int" && "$DEFAULT_MODEL" != "rrea" ]]; then
    echo -e "${RED}Error: Invalid model '${DEFAULT_MODEL}'. Must be 'bert_int' or 'rrea'.${NC}"
    exit 1
fi

# ============================================================
#  SETUP
# ============================================================
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT_ROOT="$( cd "${SCRIPT_DIR}/.." && pwd )"

BASELINE_DIR="config/experiments/massive/${DEFAULT_MODEL}_baseline"
AUGMENTED_DIR="config/experiments/massive/${DEFAULT_MODEL}_aug_red"

# ============================================================
#  BANNER
# ============================================================
term_width() { tput cols 2>/dev/null || echo 80; }
full_line() { printf '%*s\n' "$(term_width)" '' | tr ' ' "$1"; }

clear
full_line '='
printf "%*s\n" $((($(term_width) + 60) / 2)) "DAKGEA: Synthetic vs Real Data Comparison (Massive)"
full_line '='
echo ""
echo -e "${BLUE}Configuration:${NC}"
echo "  Model             : ${DEFAULT_MODEL}"
echo "  Parallel jobs     : ${DEFAULT_JOBS}"
echo "  Timeout per job   : ${DEFAULT_TIMEOUT}s"
echo "  GPU device        : ${DEFAULT_GPU_ID}"
echo "  Pattern           : ${PATTERN}"
echo ""
echo -e "${BLUE}Experiments to run:${NC}"
echo "  Baseline (Mode 1) : ${RUN_BASELINE} (${BASELINE_DIR})"
echo "  Augmented (Mode 3): ${RUN_AUGMENTED} (${AUGMENTED_DIR})"
echo ""

# Count files
if [[ "$RUN_BASELINE" == "true" ]]; then
    BASELINE_COUNT=$(find "${PROJECT_ROOT}/${BASELINE_DIR}" -name "${PATTERN}" 2>/dev/null | wc -l || echo 0)
    echo "  Baseline configs  : ${BASELINE_COUNT}"
fi

if [[ "$RUN_AUGMENTED" == "true" ]]; then
    AUGMENTED_COUNT=$(find "${PROJECT_ROOT}/${AUGMENTED_DIR}" -name "${PATTERN}" 2>/dev/null | wc -l || echo 0)
    echo "  Augmented configs : ${AUGMENTED_COUNT}"
fi

echo ""
full_line '-'

# ============================================================
#  DRY RUN
# ============================================================
if [[ "$DRY_RUN" == "true" ]]; then
    echo -e "${YELLOW}DRY RUN MODE${NC}"
    echo ""

    if [[ "$RUN_BASELINE" == "true" ]]; then
        echo -e "${BLUE}Would run BASELINE experiments:${NC}"
        echo "  bash scripts/run_experiments_parallel.sh \\"
        echo "    --dir ${BASELINE_DIR} \\"
        echo "    --jobs ${DEFAULT_JOBS} \\"
        echo "    --timeout ${DEFAULT_TIMEOUT} \\"
        echo "    --gpu-id ${DEFAULT_GPU_ID} \\"
        echo "    --pattern '${PATTERN}'"
        echo ""
    fi

    if [[ "$RUN_AUGMENTED" == "true" ]]; then
        echo -e "${BLUE}Would run AUGMENTED experiments:${NC}"
        echo "  bash scripts/run_experiments_parallel.sh \\"
        echo "    --dir ${AUGMENTED_DIR} \\"
        echo "    --jobs ${DEFAULT_JOBS} \\"
        echo "    --timeout ${DEFAULT_TIMEOUT} \\"
        echo "    --gpu-id ${DEFAULT_GPU_ID} \\"
        echo "    --pattern '${PATTERN}'"
        echo ""
    fi

    echo "After completion, analyze with:"
    echo "  python experiments/statistics/compare_baseline_augmented.py --model ${DEFAULT_MODEL}"
    echo ""
    exit 0
fi

# ============================================================
#  CONFIRMATION
# ============================================================
TOTAL_EXPERIMENTS=0
[[ "$RUN_BASELINE" == "true" ]] && TOTAL_EXPERIMENTS=$((TOTAL_EXPERIMENTS + BASELINE_COUNT))
[[ "$RUN_AUGMENTED" == "true" ]] && TOTAL_EXPERIMENTS=$((TOTAL_EXPERIMENTS + AUGMENTED_COUNT))

echo ""
read -r -p "Run ${TOTAL_EXPERIMENTS} experiments in total? [y/N] " CONFIRM
case "${CONFIRM,,}" in
    y|yes)
        ;;
    *)
        echo -e "${YELLOW}Aborted by user.${NC}"
        exit 0
        ;;
esac

# ============================================================
#  RUN EXPERIMENTS
# ============================================================
echo ""
full_line '='
echo "Starting experiments..."
echo "Started at: $(date '+%Y-%m-%d %H:%M:%S')"
full_line '='
echo ""

FAILED=0
START_TIME=$(date +%s)

# Build common args
PARALLEL_ARGS="--jobs ${DEFAULT_JOBS} --timeout ${DEFAULT_TIMEOUT} --gpu-id ${DEFAULT_GPU_ID} --pattern ${PATTERN}"
[[ "$RESUME" == "true" ]] && PARALLEL_ARGS="${PARALLEL_ARGS} --resume"

# Run baseline
if [[ "$RUN_BASELINE" == "true" ]]; then
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}Running BASELINE experiments${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo ""

    if bash scripts/run_experiments_parallel.sh \
        --dir "${BASELINE_DIR}" \
        ${PARALLEL_ARGS}; then
        echo -e "${GREEN}✓ Baseline experiments completed${NC}"
    else
        echo -e "${RED}✗ Baseline experiments failed${NC}"
        FAILED=$((FAILED + 1))
    fi
    echo ""
fi

# Run augmented
if [[ "$RUN_AUGMENTED" == "true" ]]; then
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}Running AUGMENTED experiments${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo ""

    if bash scripts/run_experiments_parallel.sh \
        --dir "${AUGMENTED_DIR}" \
        ${PARALLEL_ARGS}; then
        echo -e "${GREEN}✓ Augmented experiments completed${NC}"
    else
        echo -e "${RED}✗ Augmented experiments failed${NC}"
        FAILED=$((FAILED + 1))
    fi
    echo ""
fi

END_TIME=$(date +%s)
ELAPSED=$((END_TIME - START_TIME))

# ============================================================
#  SUMMARY
# ============================================================
echo ""
full_line '='
echo "All experiments completed!"
echo "Finished at: $(date '+%Y-%m-%d %H:%M:%S')"
echo "Total time: ${ELAPSED}s ($(printf '%02d:%02d:%02d' $((ELAPSED/3600)) $((ELAPSED%3600/60)) $((ELAPSED%60))))"
full_line '='
echo ""

if [[ ${FAILED} -gt 0 ]]; then
    echo -e "${RED}${FAILED} experiment set(s) had failures${NC}"
    echo "Check logs in results/logs/ for details"
else
    echo -e "${GREEN}All experiments completed successfully!${NC}"
fi

echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Next Steps: Analyze Results${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo "To compare baseline vs augmented performance:"
echo "  python experiments/statistics/compare_baseline_augmented.py --model ${DEFAULT_MODEL}"
echo ""
echo "To generate LaTeX tables:"
echo "  python experiments/statistics/compare_baseline_augmented.py --model ${DEFAULT_MODEL} --latex"
echo ""

full_line '='

exit ${FAILED}
