...
def name_init(x, size, initializer_range, kg, args):
    ...
    ids1 = pd.read_csv(cfg.DATA_TARGET + "/knowformer_data/" + args.dataset + "/ent_ids_1", sep="\t", header=None, names=["id", "uri"])
    ...
    names1_path = cfg.BASE_DIR + '/src/hybea/data/entity_names/' + args.SIZE_AFTER_REDUCTION_IN_PERCENTAGE + "/" + args.dataset + '/' + args.KG1_PATH_FOR_NAMES
    ...
    for k in this_vocab:
        ...
        # imdb-tmdb 3580
        # bbc-db 17013
        # if 99 < this_vocab[k] < args.VOC_LIM_2:
        if 99 < this_vocab[k] < voc_lim_2:
            uri = k
