...
def entlist2emb(Model,entids,entid2data,cuda_num, which):
    ...
    padded_alpha_i = torch.tensor(np.array(padded_alpha_i)).int().cuda(0)
...
def generate_candidate_dict(Model,train_ent1s,train_ent2s,for_candidate_ent1s,for_candidate_ent2s,
    ...
    with torch.no_grad():
        ...
        for i in range(0,len(train_ent1s),batch_size):
            temp_emb = entlist2emb(Model,train_ent1s[i:i+batch_size],entid2data, cfg.CUDA_NUM, "1").cuda(0).tolist()
            train_emb1.extend(temp_emb)
        for i in range(0,len(for_candidate_ent2s),batch_size):
            temp_emb = entlist2emb(Model,for_candidate_ent2s[i:i+batch_size],entid2data, cfg.CUDA_NUM, "2").cuda(0).tolist()
        ...
        for i in range(0,len(train_ent2s),batch_size):
            temp_emb = entlist2emb(Model,train_ent2s[i:i+batch_size],entid2data,cfg.CUDA_NUM, "2").cuda(0).tolist()
            train_emb2.extend(temp_emb)
        for i in range(0,len(for_candidate_ent1s),batch_size):
            temp_emb = entlist2emb(Model,for_candidate_ent1s[i:i+batch_size],entid2data,cfg.CUDA_NUM, "1").cuda(0).tolist()
...
def test(Model,ent_ill,entid2data,batch_size,context = "", second_mat=False, csls=cfg.CSLS):
    ...
    with torch.no_grad():
        ...
        for i in range(0,len(ents_1),batch_size):
            ...
            batch_emb_1 = entlist2emb(Model,batch_ents_1,entid2data,cfg.CUDA_NUM, "1").detach().cuda(0).tolist()
        ...
        for i in range(0,len(ents_2),batch_size):
            ...
            batch_emb_2 = entlist2emb(Model,batch_ents_2,entid2data,cfg.CUDA_NUM, "2").detach().cuda(0).tolist()
