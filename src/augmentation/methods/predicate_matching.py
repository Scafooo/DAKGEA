...
def get_predicate_label(graph, predicate) -> str:
    """Return a human-readable label for a predicate, using attribute metadata when available."""
...
def string_similarity(left: str, right: str) -> float:
    """Compute token-overlap similarity in [0, 1]."""
...
def _compute_predicate_embeddings(graph_left, graph_right, model_name: str) -> tuple[dict, SentenceTransformer]:
    """Encode predicates from two graphs with the requested SentenceTransformer model."""
...
def _predicate_similarity(
    pred_left, pred_right, emb_map, graph_left, graph_right, alpha: float
) -> float:
    """Blend semantic and lexical similarities for predicates."""
...
def match_relations_with_attrnames(
    graph_left,
    graph_right,
    entity_pairs: Iterable[Tuple[URIRef, URIRef]],
    alpha: float = 0.6,
    ...
    candidate_matches = []
    entity_pairs_set = set(entity_pairs)
    ...
    for entity_left, entity_right in entity_pairs_set:
        ...
        for (pred_left, obj_left, dir_left), (pred_right, obj_right, dir_right) in product(
            ...
            if isinstance(obj_left, URIRef) and isinstance(obj_right, URIRef) and (obj_left, obj_right) in entity_pairs_set:
...
def reduce_to_best_matches(matches):
    """Keep only the top scoring target predicate for each source predicate."""
    best = {}
...
def match_relations(dataset: Dataset):
    """Compute predicate alignments for a dataset's aligned knowledge graphs."""
    logger.info("Matching predicates between source and target graphs.")
    matches = match_relations_with_attrnames(
        dataset.knowledge_graph_source,
        dataset.knowledge_graph_target,
        dataset.aligned_entities,
    )
    reduced = reduce_to_best_matches(matches)
    logger.info(
        "Predicate matching reduced from %d to %d entries (best per source predicate).",
        len(matches),
        len(reduced),
    )
    return reduced
