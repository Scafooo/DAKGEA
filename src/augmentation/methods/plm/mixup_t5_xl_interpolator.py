"""T5 XL Mix-up Interpolator using QLoRA for RTX 4090."""
...
class MixupT5XLInterpolator:
    """
    Flan-T5-XL Interpolator ottimizzato per RTX 4090 usando QLoRA (8-bit + LoRA).
    """
    ...
    def _load_model(self):
        """Carica il modello in 8-bit e prepara LoRA."""
        logger.info(f"[T5-XL] Loading {self.model_name} in 8-bit...")
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained(
            ...
            device_map="auto", # HuggingFace gestisce la VRAM automaticamente
            torch_dtype=torch.bfloat16
        )
        
        # Prepara per il training in k-bit
        ...
        peft_config = LoraConfig(
            ...
            r=16,           # Rank degli adattatori (più alto = più capacità, più VRAM)
            lora_alpha=32,
            lora_dropout=0.05,
            target_modules=["q", "v"] # Targettiamo le proiezioni di attenzione
        )
        
        model = get_peft_model(model, peft_config)
        model.print_trainable_parameters() # Stampa quanti parametri alleniamo
        return model

    def _clean_output(self, text: str) -> str:
        text = re.sub(r'<extra_id_\d+>', '', text)
        text = re.sub(r'^.*:\s*', '', text) # Rimuove il prefisso del prompt se rigenerato
        return re.sub(r'\s+', ' ', text).strip()

    def fine_tune(self, training_rows: List[Dict[str, str]], epochs: int = 3, batch_size: int = 8, lr: float = 1e-3):
        """
        Fine-tuning con LoRA. Notare il LR più alto (1e-3) tipico per LoRA.
        """
        ...
        args = Seq2SeqTrainingArguments(
            ...
            gradient_accumulation_steps=4, # Accumulo per simulare batch più grandi
            num_train_epochs=epochs,
            learning_rate=lr,
            fp16=False,
            bf16=True, # Fondamentale su 4090
            logging_steps=20,
            save_strategy="no",
            report_to="none",
            optim="paged_adamw_32bit" # Ottimizzatore che salva memoria
        )
        ...
        logger.info("[T5-XL] Starting LoRA Fine-tuning...")
        trainer.train()
        
        # Salviamo solo gli adattatori
    ...
    def interpolate_pair(self, val_src: str, val_tgt: str, predicate: str = "", alpha: float = 0.5) -> Tuple[str, str]:
        # PROMPT RICHIESTO: "generate synthetic variation <PREDICATE>: [VALUE]"
        p_name = predicate.replace("<", "").replace(">", "").replace('_', ' ')
        ...
        self.model.eval()
        with torch.no_grad():
            # Accesso al modello base sotto LoRA per l'encoder
            # PeftModel -> BaseModel -> T5Stack (encoder)
            # Potrebbe variare leggermente in base alla versione di PEFT, usiamo get_encoder() che è sicuro
