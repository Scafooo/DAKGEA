...
class MixupT5Interpolator:
    ...
    def interpolate_pair(self, val_src: str, val_tgt: str, predicate: str = "", alpha: float = 0.5) -> Tuple[str, str]:
        # T5 Prompting: "augment [predicate]: [value]"
        p_name = predicate.replace("<", "").replace(">", "").lower()
        inputs = self.tokenizer([f"augment {p_name}: {val_src}", f"augment {p_name}: {val_tgt}"], return_tensors="pt", padding=True, truncation=True, max_length=self.max_len_in).to(self.device)
