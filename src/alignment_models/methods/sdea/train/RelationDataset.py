...
class RelationDataset(Dataset):
    ...
    def get_neighbor_matrix(facts: dict):
        lens = [len(f) if f is not None else 0 for ent, f in facts.items()]
        if not lens:
            return {}, {}, 1
        len_threshold = 1000
    ...
    def __getitem__(self, item):
        ...
        return (
            self.ent2embed1[pe1], self.ent2embed2[pe2],
            self.ent2embed1[ne1], self.ent2embed2[ne2],
            self.get_matrix(pn1, self.nm_len1, self.pad_idx1),
            self.get_matrix(pn2, self.nm_len2, self.pad_idx2),
            self.get_matrix(nn1, self.nm_len1, self.pad_idx1),
            self.get_matrix(nn2, self.nm_len2, self.pad_idx2),
            self.get_matrix(pr1, self.nm_len1, self.pad_idxr1),
            self.get_matrix(pr2, self.nm_len2, self.pad_idxr2),
            self.get_matrix(nr1, self.nm_len1, self.pad_idxr1),
            self.get_matrix(nr2, self.nm_len2, self.pad_idxr2),
        )
