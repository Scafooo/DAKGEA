...
class HybeaReader(Reader):
    ...
    def _aligned_entities_knowformer_data(self, data_dir: Path) -> Set[Tuple[URIRef, URIRef]]:
        """Load aligned entity pairs for the KnowFormer dataset layout."""
        ill_file = data_dir / self.KF_ILL
        aligned = set()
        for left, right in read_tsv(ill_file):
            aligned.add((URIRef(left), URIRef(right)))
        return aligned
