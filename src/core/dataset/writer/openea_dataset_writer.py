...
class OpeneaDatasetWriter(DatasetWriter):
    ...
    def write(self, dataset: Dataset, dir_path: str, *, dataset_name: Optional[str] = None) -> bool:
        ...
        for idx, target in enumerate(targets, 1):
            ...
            if target.name == "attribute_data":
                ...
                # Write attribute matches if available
                if dataset.attribute_matches:
                    logger.info(f"Writing {len(dataset.attribute_matches)} attribute matches")
                    self._write_attribute_matches(dataset.attribute_matches, str(destination))
            else:
    ...
    def _write_attribute_matches(self, attribute_matches: dict, dir_path: str) -> None:
        """Write attribute matches to match_attr file.

        Args:
            attribute_matches: Dict mapping source_uri -> [list of target_uris]
            dir_path: Directory path to write the file
        """
        match_file = os.path.join(dir_path, "match_attr")

        with open(match_file, 'w') as f:
            f.write("# Attribute matches\n")
            f.write("# Format: attr1 [,attr1.2,...] <TAB> attr2 [,attr2.2,...]\n")
            f.write("# Auto-generated by OpenEA dataset writer\n")
            f.write("\n")

            # Write matches sorted by source URI
            for src_uri in sorted(attribute_matches.keys()):
                tgt_uris = attribute_matches[src_uri]
                if tgt_uris:
                    tgt_list = ','.join(tgt_uris)
                    f.write(f"{src_uri}\t{tgt_list}\n")

        logger.info(f"✓ Wrote {len(attribute_matches)} attribute matches to {match_file}")
